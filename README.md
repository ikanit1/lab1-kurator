...
Открывается она блоком «Задача»: диаграмма рассеяния самого набора
`make_circles`, одна фраза о том, почему прямая здесь не работает, и список
настроек варианта — набор данных, активации, функция потерь, архитектура,
число параметров. Ниже — ссылки на семь станций, чтобы во время показа
переходить к нужному пункту сразу.
...
### Расчётная часть (`tests/test_lab1.py`, 30 тестов)

Покрывают: значения активаций и численную проверку их производных;
...
разрешимость сетью 2-2-1; корректность таблицы вариантов; актуальность
собранного `public/`.

### Интерфейс (`tests/test_ui.py`, 55 проверок в одном проходе)

Открывает собранную страницу в Chromium и нажимает каждый орган управления,
проверяя, что состояние действительно изменилось:

| что проверяется | проверок |
|---|---|
| ползунки станций 01–03 и выбор активации | 11 |
| кнопки станций 04, 06, 07 | 7 |
| станция 05: наборы данных, активация, `H`, seed, скорость, обучать / пауза / сброс | 15 |
| переключатель темы (полный цикл авто → светлая → тёмная) | 1 |
| навигация: семь ссылок ведут к своим станциям | 7 |
| подсказки на пяти графиках: появляются и скрываются | 10 |
| подписи кнопок и ползунков для доступности | 1 |
| ширина экрана 400px: нет горизонтальной прокрутки, обучение работает | 2 |
| отсутствие ошибок JavaScript | отдельным тестом |
...
Без Playwright тест пропускается, и остальные 30 по-прежнему запускаются
на голом Python с NumPy.
